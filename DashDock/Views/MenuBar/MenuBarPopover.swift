...
struct SummaryCardsView: View {
    ...
    var body: some View {
        LazyVGrid(columns: [.init(.flexible()), .init(.flexible()), .init(.flexible())], spacing: 8) {
            MetricCard(title: "Pageviews", value: data.pageviews.formattedCompact(), icon: "eye.fill", color: .purple)
            MetricCard(title: "Sessions", value: data.sessions.formattedCompact(), icon: "person.2.fill", color: .green)
            MetricCard(title: "New Users", value: data.newUsers.formattedCompact(), icon: "person.badge.plus", color: .orange)
        }
        .padding(.top, 4)
    }
}
